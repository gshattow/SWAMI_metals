...
# --------------------------------------------------------
    ...
    def StellarMassFunction(self, G):
        ...
        w = np.where(G.Type > 2)[0]
        print len(w), 'galaxies have incorrect Type'
        ...
        f = open(OutputDir + 'SMF.txt', 'w')
        for ii in range(len(counts)) :
            f.write(str(xaxeshisto[ii]) + "\t" + str(counts[ii]/ self.volume * self.Hubble_h*self.Hubble_h*self.Hubble_h / binwidth) + "\t" + str(countsRED[ii]) + "\t" + str(countsBLU[ii]) + "\n")
        f.close()
...
# ---------------------------------------------------------
    ...
    def GasMassFunction(self, G):
        ...
        f = open(OutputDir + 'GMF.txt', 'w')
        for ii in range(len(counts)) :
            f.write(str(xaxeshisto[ii]) + "\t" + str(counts[ii]) + "\t" + str(countsRED[ii]) + "\t" + str(countsBLU[ii]) + "\n")
        f.close()
...
# --------------------------------------------------------
    ...
    def MassReservoirScatter(self, G):
        ...
        plt.scatter(mvir, np.log10(G.StellarMass[w] * 1.0e10), marker='o', s=0.3, c='k', alpha=0.5)
        plt.scatter(mvir, np.log10(G.ColdGas[w] * 1.0e10), marker='o', s=0.3, color='blue', alpha=0.5)
        plt.scatter(mvir, np.log10(G.HotGas[w] * 1.0e10), marker='o', s=0.3, color='red', alpha=0.5)
        plt.scatter(mvir, np.log10(G.EjectedMass[w] * 1.0e10), marker='o', s=0.3, color='green', alpha=0.5)
        plt.scatter(mvir, np.log10(G.IntraClusterStars[w] * 1.0e10), marker='o', s=10, color='yellow', alpha=0.5)
        plt.scatter(10, 8, marker='o', s=3, c='k', alpha=1, label='Stars')
        plt.scatter(10, 8, marker='o', s=3, color='blue', alpha=1, label='Cold gas')
        plt.scatter(10, 8, marker='o', s=3, color='red', alpha=1, label='Hot gas')
        plt.scatter(10, 8, marker='o', s=3, color='green', alpha=1, label='Ejected gas')
        plt.scatter(10, 8, marker='o', s=10, color='yellow', alpha=1, label='Intracluster stars')    
...
# --------------------------------------------------------
    ...
    def MassLoadFactor(self, G):
    
        print 'Plotting the mass loading factor'
        ...
        w = np.where(G.DiskRadius > 0.)[0]
        print len(G.DiskRadius) - len(w), 'galaxies have no disk radii'
        w = np.where((G.Type == 0) & (G.ColdGas > 0.))[0]
        if(len(w) > dilute): w = sample(w, dilute)
        
        gassurfacedensity = G.ColdGas[w]/(G.DiskRadius[w]*G.DiskRadius[w]*3.14159)
        fcold = G.ColdGas[w]/(G.StellarMass[w] + G.ColdGas[w])
        e_disk = pow(gassurfacedensity/1.6e5, -0.6) * pow(fcold/0.12, 0.8)
        


        vmax = np.log10(G.Vmax[w])
        plt.scatter(10**vmax, e_disk, marker='o', s=0.3, c='b', edgecolor = 'b', alpha=0.5, label = r'$\epsilon_{disk}$')
        
        w = np.where(e_disk < 1.0e3)[0]
        vmax = vmax[w]
        e_disk = e_disk[w]
       
        z = np.polyfit(vmax, np.log10(e_disk), 1)
        print z
        vm = np.linspace(1.5, 3.5)
        ed = vm*z[0] + z[1]
        plt.plot(10**vm, 10**ed)
        
        e_disk_calc = vmax*z[0] + z[1]
        sdev = np.sqrt(np.sum((e_disk - e_disk_calc)**2))/float(len(vmax))
        print 'standard deviation is', sdev

        plt.ylabel(r'$\epsilon_{disk}$')  # Set the y...
        plt.xlabel(r'V$_{\mathrm{max}}$')  # and the x-axis labels
        
        plt.axis([30, 1500, 0.2, 200])
        ax.set_xscale('log')
        ax.set_yscale('log')
...
#        plt.text(13.5, 8.0, r'$\mathrm{All}$')
        ...
        outputFile = OutputDir + '0.MassLoadFactor' + OutputFormat
        plt.savefig(outputFile)  # Save the figure
        print 'Saved file to', outputFile
        plt.close()
...
# --------------------------------------------------------
    ...
    def VvirVmax(self, G):
        ...
        vvir = np.log10(G.Vvir[w])
        vmax = np.log10(G.Vmax[w])
        plt.scatter(vvir, vmax, marker='o', s=0.3, c='m', edgecolor = 'b', alpha=0.5, label = r'galaxies')
#        plt.scatter(vvir, vvir/3. + 1.6, marker='o', s=0.3, c='m', edgecolor = 'k', alpha=0.5, label = r'galaxies')
        
        vv = np.linspace(np.min(vvir), np.max(vvir), 100)
        z = np.polyfit(vvir, vmax, 1)
        print z
        vm = vv*z[0] + z[1]
        plt.plot(vv, vm)

        z2 = np.polyfit(vmax, vvir, 1)
        print z2


        plt.ylabel(r'$V_{max}$')  # Set the y...
        plt.xlabel(r'$V_{vir}$')  # and the x-axis labels
...
#        plt.text(13.5, 8.0, r'$\mathrm{All}$')
        ...
        outputFile = OutputDir + '0.VvirVmax' + OutputFormat
        plt.savefig(outputFile)  # Save the figure
        print 'Saved file to', outputFile
        plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        '--dir_name',
        dest='DirName',
        default='./millennium_W11/',
        help='input directory name (default:./millennium_WXY/)',
        metavar='DIR',
        )
    parser.add_option(
        '-f',
